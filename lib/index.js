...
function onmousedown(state,e ){
	if(state.container().contains(e.srcElement)){
...
function onmousemove(state, e){
	...
	const historyDragging = e.touches
	...
	if( state.selectingText() ){
		...
		if(touches.length > 1){
			...
			if(state.mousedown() > 2){
				if( state.settings.ignoreTextSelect && !state.selectingText() ){
					state.selectingText(
			...
			if(!state.selectingText() && state.mousedown() > 3){

				state.dragging(state.mousedown())
				var movement =
					{ x: (x - state.move().x) * state.polarity().x
					, y: (y - state.move().y) * state.polarity().y
					}
...
function Pointer(options){
	...
	options.container = options.container || always(document.body)

	options.coords = options.coords || defaultOptions.coords
	options.polarity = options.polarity || { x: 1, y: 1 }
	options.ignoreTextSelect = options.ignoreTextSelect || false

	const selectingText = stream(false)
	...
	/**
	 * Flip polarity for x/y axis to change behavour of event handlers
	 */
	var polarity = stream(options.polarity)
	/*
		...
		merge(
			{
				settings: {
					...
					,ignoreTextSelect: options.ignoreTextSelect
				}
