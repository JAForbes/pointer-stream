...
var ifElse = function(p, a, b){
	return function(){
		return (p.apply(null, arguments) ? a : b)
			.apply(null, arguments)
	}
}

var anyPass = function(ps){
	return function(v){
		return ps.some(function(f){
			return f(v)
		})
	}
}
...
function Pointer(options){
	...
	options.mousewheelContainer = options.mousewhellContainer || always(window)
	...
	*/
	 ...
	 */
	var notHover = anyPass([dragging,pinching,wheeling])
	...
	*/
	...
	var state =
		merge(
			{
				settings: { scale: { min:min, max:max }, offset:offset }
